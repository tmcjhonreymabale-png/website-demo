...
$current_page = 'history_logs';

// Export function - defined ONCE at the top
function exportLogs($db) {
    ...
    $query = "SELECT l.created_at, a.username, a.first_name, a.last_name, a.admin_type as role, l.action, l.description, l.ip_address
              FROM admin_logs l
    ...
    // Headers
    fputcsv($output, ['Date & Time', 'Admin', 'Username', 'Role', 'Action', 'Description', 'IP Address']);
    ...
    foreach ($logs as $log) {
        fputcsv($output, [
            ...
            $log['username'],
            $log['role'] == 'main_admin' ? 'Main Admin' : 'Staff',
            $log['action'],
            $log['description'],
            $log['ip_address']
        ]);
...
// Build query
$query = "SELECT l.*, a.username, a.first_name, a.last_name, a.admin_type as role 
          FROM admin_logs l
          LEFT JOIN admins a ON l.admin_id = a.id
          WHERE 1=1";
...
$admins = [];
try {
    $admins_query = "SELECT id, username, first_name, last_name FROM admins ORDER BY first_name";
    $admins_stmt = $db->query($admins_query);
...
<div class="admin-management">
    ...
    <div class="page-header">
        ...
        <p>Track all admin activities in the system</p>
    </div>

    <!-- Navigation Tabs -->
    <div class="tabs-container">
        <div class="tabs">
            <a href="logs.php" class="tab-btn active">
                <span class="material-icons">admin_panel_settings</span>
                Admin Logs
            </a>
            <a href="resident_logs.php" class="tab-btn">
    ...
    <div class="filter-section">
        <form method="GET" class="filter-form">
            ...
            <div class="filter-group">
                ...
                <input type="date" name="from" id="from" value="<?php echo htmlspecialchars($date_from); ?>">
            </div>
            
            <div class="filter-group">
                <label>📅 TO DATE</label>
                <input type="date" name="to" id="to" value="<?php echo htmlspecialchars($date_to); ?>">
            </div>
            ...
            <div class="filter-actions">
                ...
                <a href="?export=csv&admin=<?php echo urlencode($admin_filter); ?>&action=<?php echo urlencode($action_filter); ?>&from=<?php echo urlencode($date_from); ?>&to=<?php echo urlencode($date_to); ?>" class="btn-export">
                ...
                <tbody>
                    ...
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="admin-name"><?php echo htmlspecialchars(($log['first_name'] ?? '') . ' ' . ($log['last_name'] ?? '')); ?></div>
                                <div class="admin-username">@<?php echo htmlspecialchars($log['username'] ?? ''); ?></div>
                            </td>
                            <td>
                                <span class="role-badge <?php echo ($log['role'] ?? '') == 'main_admin' ? 'main' : 'staff'; ?>">
                                    <?php echo ($log['role'] ?? '') == 'main_admin' ? 'Main Admin' : 'Staff'; ?>
                                </span>
                            </td>
